...
cp .env.example .env           # SONIOX_API_KEY 입력 (또는 60db 사용 시 SIXTYDB_API_KEY)
bash scripts/dub.sh "https://www.youtube.com/watch?v=..."
```

### 선택: STT 엔진 (Soniox 기본 / 60db)

받아쓰기(STT) 엔진을 `DUBYDUBY_STT` 환경변수로 고를 수 있습니다. 기본값은 `soniox`라서 아무것도 안 바꾸면 기존과 동일하게 동작합니다.

| 엔진 | 값 | 키 | 비고 |
|---|---|---|---|
| Soniox (기본) | `soniox` | `SONIOX_API_KEY` | 비동기 batch, 월 200분 무료 |
| 60db | `60db` | `SIXTYDB_API_KEY` | `POST https://api.60db.ai/stt` 단일 호출, 화자 분리 포함 |

```bash
# .env 에 추가
DUBYDUBY_STT=60db
SIXTYDB_API_KEY=sk_...
# DUBYDUBY_STT_LANG=en       # 소스 언어 (기본 en, "auto"로 자동 감지)

bash scripts/dub.sh "https://www.youtube.com/watch?v=..."   # 평소처럼 실행
```

두 엔진 모두 **동일한** `2_transcript/{tokens.json, transcript.md}`를 생성하므로 번역·합성·자막 등 이후 단계는 전부 그대로 동작합니다 (`scripts/transcribe_60db.py`가 60db 응답을 Soniox 스키마로 변환). 단, **60db는 STT 전용** — TTS는 60db에 한국어 보이스가 없어 Supertonic을 그대로 씁니다. 또한 60db `/stt`는 업로드 **10MB / 1시간** 제한이 있어, 긴 소스는 mono 64kbps로 자동 재인코딩하고 그래도 크면 경고합니다 (`dub.sh <URL> <초>`로 잘라서 처리).
...
YouTube URL
  ...
  ▼ Soniox | 60db STT (+화자 분리) 2_transcript/{tokens.json, transcript.md}
  │   (DUBYDUBY_STT=soniox 기본 / 60db 선택 — 동일 스키마 출력)
...
- [Soniox](https://soniox.com) 계정 — 월 200분 무료, 이후 유료. `.env`로 BYOK. (또는 `DUBYDUBY_STT=60db` + [60db](https://60db.ai) 키)
...
```
dubyduby/
├── scripts/             # 파이프라인: setup, dub, transcribe(_60db), synthesize, place, subtitle, finalize
...
- [60db](https://60db.ai) — 대체 STT API (선택)
